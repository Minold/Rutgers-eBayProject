...
<body style="font-family: Arial, sans-serif;">
    ...
    <div style="text-align: center;">
        <h1 style="color: blue;">Subcategory: Others</h1>
        <table align="center">
            <tr>  
        ...
        <div class="container" style="margin-top: 20px;">
            <form class="form-inline" method="post" action="search.jsp?subcat=Others">
            ...
            <form class="form-inline" method="post" action="Others.jsp">
        ...
        <% 
            ...
            if(sortParam == null){
                resultset = stmt.executeQuery("SELECT * from listings WHERE subcategory='Others';");
            }else if(sortParam.equals("Name")){
                resultset = stmt.executeQuery("SELECT * from listings WHERE subcategory='Others' ORDER BY itemname;");
            }else if(sortParam.equals("lowToHigh")){
                resultset = stmt.executeQuery("SELECT * from listings WHERE subcategory='Others' ORDER BY price;");
            }else if(sortParam.equals("highToLow")){
                resultset = stmt.executeQuery("SELECT * from listings WHERE subcategory='Others' ORDER BY price DESC;");
            }else if(sortParam.equals("Tag")){
                resultset = stmt.executeQuery("SELECT * from listings WHERE subcategory='Others' ORDER BY CAST(subattribute as SIGNED INTEGER);");
            }else if(sortParam.equals("Open")){
                resultset = stmt.executeQuery("SELECT * from listings WHERE subcategory='Others' AND closed='0' ORDER BY itemname;");
            }else if(sortParam.equals("Closed")){
                resultset = stmt.executeQuery("SELECT * from listings WHERE subcategory='Others' AND closed='1' ORDER BY itemname;");
            }else{
                resultset = stmt.executeQuery("SELECT * from listings WHERE subcategory='Others';");
            }
        %>
